...
const FamilyButtonContainer: FC<FamilyButtonContainerProps> = ({
  ...
  return (
    <motion.div
      className={cn(
        "relative z-10 flex cursor-pointer flex-col items-center space-y-1 border border-white/10 text-foreground shadow-lg",
        !isExpanded ? "bg-gradient-to-b from-default-100 to-default-50" : ""
      )}
      layoutRoot
      layout
      ...
      >
        {isExpanded ? (
          <motion.div
            className={cn(
              "group",
              button({ variant: "flat", color: "danger", isIconOnly: true, size: "lg" })
            )}
          ...
          <motion.div
            className={cn(
              "group",
              button({ variant: "shadow", color: "primary", isIconOnly: true, size: "lg" })
            )}
